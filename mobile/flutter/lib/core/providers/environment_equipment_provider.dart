...
enum WorkoutEnvironment {
  ...
  List<String> get defaultEquipment {
    switch (this) {
      case WorkoutEnvironment.commercialGym:
        // Exhaustive commercial-gym list — every piece the app recognises
        // (snake_case ids + the master "databaseEquipment" labels from
        // `EquipmentSearchSheet`), so a freshly-created Commercial Gym
        // profile unlocks everything the user actually has access to.
        // Onboarding's `kCommercialGymEquipmentPreset` is the source of
        // truth for the union of both forms; this list omits only the
        // onboarding-only `full_gym` preset marker. Keep them in sync.
        return [
          // Free weights
          'bodyweight',
          'barbell',
