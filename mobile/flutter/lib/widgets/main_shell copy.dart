...
class _DraggableAICoachButtonState extends State<_DraggableAICoachButton> {
  ...
  Widget build(BuildContext context) {
    ...
    // Account for floating nav bar: 24 margin + 52 height + 10 bottom + blur area
    const floatingNavBarOffset = 86.0;
    if (_xPosition < 0 || _yPosition < 0) {
      _xPosition = size.width - buttonSize - 16;
      _yPosition = size.height - buttonSize - bottomPadding - floatingNavBarOffset - 40; // Above nav bar
    }

    // Boundaries for dragging - allow going lower (just above floating nav bar)
    const minX = 8.0;
    final maxX = size.width - buttonSize - 8;
    final minY = topPadding + 60;
    final maxY = size.height - buttonSize - bottomPadding - floatingNavBarOffset - 12; // Above floating nav bar
...
class _FloatingNavBarWithAI extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    // Liquid glass effect - very transparent with strong blur
    ...
    // Nav bar dimensions - slim floating bar
    const navBarHeight = 52.0;
    const itemHeight = 40.0;
    const plusButtonSize = 44.0;
    const plusButtonProtrusion = 8.0; // How much the + button sticks up

    return SizedBox(
      height: navBarHeight + plusButtonProtrusion,
      child: Stack(
        clipBehavior: Clip.none,
        children: [
          // Floating nav bar container with blur
          Positioned(
            left: 0,
            right: 0,
            bottom: 0,
          ...
          Positioned(
            left: 0,
            right: 0,
            bottom: (navBarHeight - plusButtonSize) / 2 + plusButtonProtrusion,
            child: Center(
