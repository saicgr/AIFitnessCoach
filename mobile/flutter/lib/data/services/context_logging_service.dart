...
class ContextLoggingService {
  ...
  /// Log when user starts a calibration workout
  Future<void> logCalibrationStarted(String calibrationId) async {
    try {
      final userId = await _apiClient.getUserId();
      if (userId == null) return;
      ...
      debugPrint('✅ [ContextLog] Logged calibration started: $calibrationId');
    } catch (e) {
      debugPrint('⚠️ [ContextLog] Failed to log calibration started: $e');
    }
  }

  /// Log when user completes a calibration workout
  Future<void> logCalibrationCompleted(
    ...
    try {
      ...
      debugPrint('✅ [ContextLog] Logged calibration completed: $calibrationId');
    } catch (e) {
      debugPrint('⚠️ [ContextLog] Failed to log calibration completed: $e');
    }
  }

  /// Log when user skips calibration
  Future<void> logCalibrationSkipped() async {
    try {
      final userId = await _apiClient.getUserId();
      if (userId == null) return;

      await _logEvent(
        eventType: 'calibration_skipped',
        eventData: {},
        context: {
          'time_of_day': _getTimeOfDay(),
          'day_of_week': _getDayOfWeek(),
      ...
      debugPrint('✅ [ContextLog] Logged calibration skipped');
    } catch (e) {
      debugPrint('⚠️ [ContextLog] Failed to log calibration skipped: $e');
    }
  }

  /// Log when user accepts calibration adjustments
  Future<void> logCalibrationAdjustmentsAccepted(
    ...
    try {
      ...
      await _logEvent(
        eventType: 'calibration_adjustments_accepted',
        eventData: {
      ...
      debugPrint('✅ [ContextLog] Logged calibration adjustments accepted');
    } catch (e) {
      debugPrint('⚠️ [ContextLog] Failed to log calibration adjustments accepted: $e');
    }
  }

  /// Log when user declines calibration adjustments
  Future<void> logCalibrationAdjustmentsDeclined() async {
    try {
      final userId = await _apiClient.getUserId();
      if (userId == null) return;

      await _logEvent(
        eventType: 'calibration_adjustments_declined',
        eventData: {},
        context: {
          'time_of_day': _getTimeOfDay(),
          'day_of_week': _getDayOfWeek(),
      ...
      debugPrint('✅ [ContextLog] Logged calibration adjustments declined');
    } catch (e) {
      debugPrint('⚠️ [ContextLog] Failed to log calibration adjustments declined: $e');
    }
  }
