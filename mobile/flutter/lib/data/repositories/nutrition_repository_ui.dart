...
extension NutritionRepositoryExt on NutritionRepository {
  ...
  Future<LogFoodResponse> logAdjustedFood({
    ...
    /// rapid double-tap of "Log" — or an offline-queued meal replayed after
    /// reconnect — cannot create two food_log rows. Generated by the caller
    ...
    // (A11) Offline → persist the request body verbatim to the meal write
    // queue and return a synthetic response built from the (client-known)
    // macros so the caller's optimistic UI splice still works. The queued
    ...
    if (!await NutritionRepository.isOnline()) {
      ...
      return LogFoodResponse.fromJson({
        'success': true,
        'food_log_id': null, // not yet persisted server-side
        'food_items': foodItems,
        'total_calories': totalCalories,
        'protein_g': totalProtein,
        'carbs_g': totalCarbs,
        'fat_g': totalFat,
        if (totalFiber != null) 'fiber_g': totalFiber,
        'source_type': sourceType,
