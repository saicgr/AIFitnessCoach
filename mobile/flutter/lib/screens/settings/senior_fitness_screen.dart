...
class _SeniorFitnessScreenState extends ConsumerState<SeniorFitnessScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: st.isLoading ? const _SettingsFormSkeleton() : SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 12),
        _buildToggle('Extended Warmup', 'Longer warmup for joint preparation', st.extendedWarmup, (v) => ref.read(seniorSettingsProvider.notifier).setExtendedWarmup(v), Icons.timer, context.accentColor, d, tp, tm, el),
        const SizedBox(height: 12),
        _buildToggle('Joint-Friendly Exercises', 'Prioritize low-impact movements', st.jointFriendlyExercises, (v) => ref.read(seniorSettingsProvider.notifier).setJointFriendly(v), Icons.accessibility, context.accentColor, d, tp, tm, el),
        const SizedBox(height: 12),
        _buildToggle('Balance Exercises', 'Include stability and balance work', st.balanceExercises, (v) => ref.read(seniorSettingsProvider.notifier).setBalanceExercises(v), Icons.balance, context.accentColor, d, tp, tm, el),
        const SizedBox(height: 12),
        _buildToggle('Reduced Impact', 'Avoid jumping and high-impact moves', st.reducedImpact, (v) => ref.read(seniorSettingsProvider.notifier).setReducedImpact(v), Icons.do_not_step, AppColors.success, d, tp, tm, el),  // accent-allowlist: success/positive state - must stay green regardless of accent
  ...
  Widget _buildInfoCard(bool d, Color tp, Color tm, Color el) => Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: context.accentColor.withOpacity(0.1), borderRadius: BorderRadius.circular(16), border: Border.all(color: context.accentColor.withOpacity(0.3))), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(Icons.info_outline, color: context.accentColor, size: 24), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(AppLocalizations.of(context).seniorFitnessAgeAdaptedWorkouts, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: tp)), const SizedBox(height: 4), Text(AppLocalizations.of(context).seniorFitnessTheseSettingsHelpCustomize, style: TextStyle(fontSize: 14, color: tm, height: 1.4))]))]));
  ...
  Widget _buildRecoverySlider(SeniorSettingsState st, bool d, Color tp, Color tm, Color el) {
    ...
    return Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: el, borderRadius: BorderRadius.circular(12)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('Recovery Multiplier', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: tp)), Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), decoration: BoxDecoration(color: context.accentColor.withOpacity(0.15), borderRadius: BorderRadius.circular(8)), child: Text('${st.recoveryMultiplier}x', style: TextStyle(color: context.accentColor, fontWeight: FontWeight.w600)))]), const SizedBox(height: 12), Slider(value: st.recoveryMultiplier, min: 1.0, max: 2.0, divisions: 4, activeColor: context.accentColor, onChanged: (v) => ref.read(seniorSettingsProvider.notifier).setRecoveryMultiplier(v)), Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: labels.entries.map((e) => Text(e.value, style: TextStyle(fontSize: 10, color: tm))).toList())]));
  }

  Widget _buildRestSlider(SeniorSettingsState st, bool d, Color tp, Color tm, Color el) => Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: el, borderRadius: BorderRadius.circular(12)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(AppLocalizations.of(context).seniorFitnessRestBetweenSets, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: tp)), Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), decoration: BoxDecoration(color: context.accentColor.withOpacity(0.15), borderRadius: BorderRadius.circular(8)), child: Text(AppLocalizations.of(context)!.seniorFitnessScreenS(st.restBetweenSets), style: TextStyle(color: context.accentColor, fontWeight: FontWeight.w600)))]), const SizedBox(height: 12), Slider(value: st.restBetweenSets.toDouble(), min: 60, max: 180, divisions: 6, activeColor: context.accentColor, onChanged: (v) => ref.read(seniorSettingsProvider.notifier).setRestBetweenSets(v.round())), Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: ['60s', '90s', '120s', '150s', '180s'].map((t) => Text(t, style: TextStyle(fontSize: 10, color: tm))).toList())]));
  ...
  Widget _buildSaveButton(SeniorSettingsState st, bool d) => SizedBox(width: double.infinity, height: 56, child: ElevatedButton(onPressed: st.isSaving ? null : () async { await ref.read(seniorSettingsProvider.notifier).saveSettings(); if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(AppLocalizations.of(context).seniorFitnessSettingsSaved))); }, style: ElevatedButton.styleFrom(backgroundColor: context.accentColor, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))), child: st.isSaving ? const SizedBox(width: 24, height: 24, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : Text(AppLocalizations.of(context).seniorFitnessSaveSettings, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600))));
}
