...
class _SeniorFitnessScreenState extends ConsumerState<SeniorFitnessScreen> {
  ...
  Widget _buildRestSlider(SeniorSettingsState st, bool d, Color tp, Color tm, Color el) => Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: el, borderRadius: BorderRadius.circular(12)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(AppLocalizations.of(context).seniorFitnessRestBetweenSets, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: tp)), Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), decoration: BoxDecoration(color: AppColors.purple.withOpacity(0.15), borderRadius: BorderRadius.circular(8)), child: Text(AppLocalizations.of(context)!.seniorFitnessScreenS(st.restBetweenSets), style: TextStyle(color: AppColors.purple, fontWeight: FontWeight.w600)))]), const SizedBox(height: 12), Slider(value: st.restBetweenSets.toDouble(), min: 60, max: 180, divisions: 6, activeColor: AppColors.purple, onChanged: (v) => ref.read(seniorSettingsProvider.notifier).setRestBetweenSets(v.round())), Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: ['60s', '90s', '120s', '150s', '180s'].map((t) => Text(t, style: TextStyle(fontSize: 10, color: tm))).toList())]));
  ...
  Widget _buildSaveButton(SeniorSettingsState st, bool d) => SizedBox(width: double.infinity, height: 56, child: ElevatedButton(onPressed: st.isSaving ? null : () async { await ref.read(seniorSettingsProvider.notifier).saveSettings(); if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(AppLocalizations.of(context).seniorFitnessSettingsSaved))); }, style: ElevatedButton.styleFrom(backgroundColor: AppColors.cyan, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))), child: st.isSaving ? const SizedBox(width: 24, height: 24, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : Text(AppLocalizations.of(context).seniorFitnessSaveSettings, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600))));
}
