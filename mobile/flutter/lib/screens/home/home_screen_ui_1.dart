...
extension _HomeScreenStateUI1 on _HomeScreenState {
    ...
    void renderTileGroup(List<HomeTile> tiles) {
      ...
      for (final tile in tiles) {
        ...
        if (tile.type == TileType.nextWorkout) {
          ...
          // Hero workout section moved to the unified-home `HomeWorkoutCard`
          // sliver (see `_widgetForSection`). This legacy tile-layout path is
          // dead; emit nothing so the old SectionedHeroArea anchor isn't
          // mounted (it would duplicate the live carousel's GlobalKey).
          continue;
        }
  ...
  List<Widget> _buildFallbackTilesAsSlivers(
    ...
    return [
      // Hero workout section moved to the unified-home `HomeWorkoutCard`
      // sliver. This legacy fallback path no longer renders SectionedHeroArea
      // (its tour anchor would duplicate the live carousel's GlobalKey).
      // Quick actions row
      SliverToBoxAdapter(
        child: Padding(
