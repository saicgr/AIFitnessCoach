...
import '../../data/models/dish_image.dart';
...
class MenuAnalysisSheet extends ConsumerStatefulWidget {
  ...
  final bool isDark;
  /// Persist the ticked dishes. MUST resolve to true only when the write
  /// actually succeeded — the sheet shows "Logged" on the strength of it.
  ///
  /// This returns `Future<bool>` rather than void because a void callback let
  /// two callers pass an empty stub: the button reported success while nothing
  /// was written. A caller that cannot log must now say so by returning false.
  final Future<bool> Function(List<Map<String, dynamic>>) onLogItems;
  ...
  factory MenuAnalysisSheet.fromSharedPhoto({
    ...
    required Future<bool> Function(List<Map<String, dynamic>>) onLogItems,
...
class _MenuAnalysisSheetState extends ConsumerState<MenuAnalysisSheet> {
  ...
  // Per-dish personal notes the user pinned before logging, keyed by
  // MenuItem.id. Seeded from the menu's own printed description; edited via
  // the bookmark affordance on each card. Written to food_logs.notes on log.
  final Map<String, String> _dishNotes = {};

  // Resolved dish thumbnails, keyed by MenuItem.id, and which dish names have
  // already been sent to the resolver so streamed pages don't re-request.
  final Set<String> _imageResolveRequested = {};
  // Dishes with an AI generation in flight (drives the per-card spinner).
  final Set<String> _imageGenerating = {};
  // Names we've auto-generated for so the Recommended auto-pass fires once.
  ...
  void initState() {
    ...
    _loadPersistedSort();
    // Seed each dish's note from its printed menu description, and kick off
    // free-source thumbnail resolution for whatever we already have.
    _seedNotesFromDescriptions(_items);
    WidgetsBinding.instance.addPostFrameCallback((_) => _resolveDishImages());
  }

  /// A dish's menu description is its default note — logging it should keep
  /// "what the dish was" without the user typing anything. An explicit edit
  /// overrides it; this never clobbers an edit already in the map.
  void _seedNotesFromDescriptions(List<MenuItem> items) {
    for (final item in items) {
      final desc = item.description;
      if (desc != null && desc.isNotEmpty && !_dishNotes.containsKey(item.id)) {
        _dishNotes[item.id] = desc;
      }
    }
  }
  ...
  void _onStreamingUpdate() {
    ...
    if (pending.isNotEmpty) {
      ...
      _seedNotesFromDescriptions(more);
      _refreshRecommendation();
      // Resolve thumbnails for the pages that just streamed in.
      _resolveDishImages();
    } else {
      setState(() {});
    }
  ...
  /// Resolve dish thumbnails from FREE sources for every not-yet-requested
  /// dish, then auto-generate images for the Recommended picks only.
  ///
  /// Free resolution is safe to run over the whole menu (it costs nothing);
  /// AI generation is gated to the ~8 dishes the user is most likely to look
  /// at, plus anything they tap. Bill mode skips images — a receipt line has
  /// no dish worth illustrating and the estimate call is already the cost.
  Future<void> _resolveDishImages() async {
    if (!mounted || widget.userId == null) return;
    if (widget.analysisType == 'bill') return;

    final pending = [
      for (final item in _items)
        if (!_imageResolveRequested.contains(item.id) &&
            (item.dishImageUrl == null || item.dishImageUrl!.isEmpty))
          item,
    ];
    if (pending.isEmpty) {
      _maybeAutoGenerateRecommended();
      return;
    }
    for (final item in pending) {
      _imageResolveRequested.add(item.id);
    }

    try {
      final repo = ref.read(nutritionRepositoryProvider);
      final byName = await repo.resolveDishImages(
        names: [for (final i in pending) i.name],
      );
      if (!mounted || byName.isEmpty) {
        _maybeAutoGenerateRecommended();
        return;
      }
      setState(() {
        for (var i = 0; i < _items.length; i++) {
          final img = byName[_items[i].name];
          if (img != null && (_items[i].dishImageUrl ?? '').isEmpty) {
            _items[i] = _items[i].copyWith(
              dishImageUrl: img.url,
              dishImageCaption: img.requiredCaption,
            );
          }
        }
      });
    } catch (_) {
      // A thumbnail is decoration — never surface a failure.
    }
    _maybeAutoGenerateRecommended();
  }

  /// Auto-generate images for the Recommended dishes that still have none.
  /// Runs once per sheet; every other dish generates only on an explicit tap.
  Future<void> _maybeAutoGenerateRecommended() async {
    if (_autoGenerated || !mounted || widget.userId == null) return;
    final picks = _recommendation?.picks;
    if (picks == null || picks.isEmpty) return;
    _autoGenerated = true;

    for (final pick in picks.take(8)) {
      final idx = _items.indexWhere((i) => i.name == pick.item.name);
      if (idx < 0) continue;
      final item = _items[idx];
      if ((item.dishImageUrl ?? '').isNotEmpty) continue;
      // Sequential, best-effort: stop quietly the moment the cap is hit
      // rather than firing eight doomed requests.
      final ok = await _generateDishImage(item, silentCap: true);
      if (!ok) break;
    }
  }

  /// Generate one dish image. Returns false when the daily cap is reached so
  /// the auto-pass can stop. [silentCap] suppresses the cap snackbar (the
  /// auto-pass shouldn't nag; an explicit tap should).
  Future<bool> _generateDishImage(MenuItem item, {bool silentCap = false}) async {
    if (widget.userId == null || _imageGenerating.contains(item.id)) return true;
    setState(() => _imageGenerating.add(item.id));
    try {
      final repo = ref.read(nutritionRepositoryProvider);
      final img = await repo.generateDishImage(
        name: item.name,
      ...
      return true;
    } on DishImageException catch (e) {
      if (e.isCapReached) {
        if (!silentCap && mounted) {
      ...
      if (mounted) setState(() => _imageGenerating.remove(item.id));
    }
  }

  /// Edit the personal note pinned to a dish. Pre-filled with the current note
  /// (which itself defaults to the menu description). Saved to food_logs.notes
  /// when the dish is logged.
  Future<void> _editDishNote(MenuItem item) async {
    final controller = TextEditingController(text: _dishNotes[item.id] ?? '');
    final saved = await showDialog<String>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Note for this dish'),
        content: TextField(
          ...
          decoration: const InputDecoration(
            hintText: 'e.g. asked for no butter, extra crispy',
          ),
        ),
        actions: [
          TextButton(
    ...
    setState(() {
      if (saved.isEmpty) {
        _dishNotes.remove(item.id);
      } else {
        _dishNotes[item.id] = saved;
      }
    });
  }
  ...
  Color _shareAccent() {
    ...
    return AccentColorScope.of(context).getColor(isDark);
  }

  /// Share one dish straight off the scan — works before anything is logged.
  Future<void> _shareDish(MenuItem item) async {
    final shareable = NutritionAdapter.fromMenuItem(
      item,
      restaurantName: widget.restaurantName,
      accent: _shareAccent(),
    );
    if (shareable == null || !mounted) return;
    await ShareableSheet.show(context, data: shareable);
  }

  /// Share the whole scan — the current selection if any, else the menu.
  Future<void> _shareMenu() async {
    final shareable = NutritionAdapter.fromMenuAnalysis(
      restaurantName: widget.restaurantName,
      items: _items,
      selectedIds: _selected,
      accent: _shareAccent(),
      menuPhotoUrls: _photoUrls,
    );
    if (shareable == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  /// "What else would round this out?" — a deterministic, instant pass over
  /// the menu's remaining dishes against the calorie/protein left in the
  /// user's budget after the current selection. No AI call.
  Future<void> _completeMyMeal() async {
    final remainingBudget = _remainingBudgetCalories();
    final suggestions = MenuRecommendationService.completeMyMeal(
      allItems: _items,
      selectedIds: _selected,
    ...
    if (suggestions.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Nothing on this menu fits what you have left.'),
        ),
      );
      return;
    }
  ...
  /// Calories the user has left today, from the budget rings' data, or null
  /// when no target is configured (the ranker then ignores the budget axis).
  double? _remainingBudgetCalories() {
    final state = ref.read(nutritionPreferencesProvider);
    if (!state.hasConfiguredTargets) return null;
    final target = state.currentCalorieTarget;
    if (target == null) return null;
    return target.toDouble() - _selectedTotals.cal;
  }
  ...
  Future<void> _handleAdjustDish(MenuItem item, {bool openOnAddons = false}) async {
    if (!_canAdjustDishes) return;
    final menuAddons = _menuAddonsFor(item);
    // Only fall back to companion suggestions when the menu itself listed no
    // add-ons — otherwise the real menu sides are the better offer.
    List<CompanionSuggestion> companions = const [];
    if (menuAddons.isEmpty && !item.isAddon) {
      companions = await ref.read(nutritionRepositoryProvider).fetchCompanions(
            userId: widget.userId!,
            primaryFoodName: item.name,
            mealType: widget.mealType!,
          );
      if (!mounted) return;
    }
  ...
  Map<String, dynamic> _payloadForItem(MenuItem item) {
    ...
    if (adj?.refinedDishOverride != null) {
      ...
      return {
        ...
        if (_dishNotes[item.id] != null) 'note': _dishNotes[item.id],
      };
    }
  ...
  /// Add-on items from THIS menu that the user could attach to [item] — the
  /// sauces / sides / enhancements the scan surfaced, minus add-ons already
  /// selected as standalone rows so nothing is offered twice.
  List<MenuItem> _menuAddonsFor(MenuItem item) {
    if (item.isAddon) return const [];
    return [
      for (final candidate in _items)
        if (candidate.isAddon && !_selected.contains(candidate.id)) candidate,
    ];
  }
  ...
  Future<void> _handleLog() async {
    // `_logging` blocks a second tap during the await — without it a double-tap
    // fires two writes and duplicates every ticked dish.
    if (_logged || _logging || _selected.isEmpty) return;
    ...
    bool ok = false;
    try {
      ok = await widget.onLogItems(payload);
    } finally {
      // The fresh-scan callers pop this sheet on success, so it may already be
      // gone by the time the write resolves — guard before touching state.
      if (mounted) {
        setState(() {
          _logging = false;
          // Only claim "Logged" when the write actually landed. Reporting
          // success on a failed write is what made the saved-menu stub
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          IconButton(
            tooltip: 'Share',
            icon: Icon(Icons.ios_share_rounded, color: colors.textSecondary),
  ...
  /// Single builder for a dish card so every list (recommended / by-section /
  /// flat) wires images, notes, share and adjust identically. An add-on row
  /// hides the "add-ons" affordance (you don't attach add-ons to a sauce) and
  /// the note/share affordances only appear in a real logging context.
  Widget _buildItemCard(MenuItem item) {
    final canImage = widget.userId != null && widget.analysisType != 'bill';
    return MenuAnalysisItemCard(
      item: item,
      ...
      onAdjust: _canAdjustDishes ? () => _handleAdjustDish(item) : null,
      adjustmentSummary: _dishAdjustments[item.id]?.summary,
      onRequestImage: canImage ? () => _generateDishImage(item) : null,
      isGeneratingImage: _imageGenerating.contains(item.id),
      onShare: () => _shareDish(item),
      onEditNote: _canAdjustDishes ? () => _editDishNote(item) : null,
      pinnedNote: _dishNotes[item.id],
    );
  }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          // "Complete this meal" — a deterministic, instant pass that suggests
          // menu dishes to round out the plate. Hidden for bills (a receipt is
          // already what was ordered — nothing to complete) and the chat flow.
          if (showAddFood && widget.analysisType != 'bill' && !_logged) ...[
            Center(
              child: blurPill(
                Material(
                  color: pillTint,
                ...
                child: KeyedSubtree(
                  ...
                  child: blurPill(
                    FilledButton.icon(
                      ...
                      onPressed: (_selected.isEmpty || _logged || _logging)
                          ? null
                          : _handleLog,
...
/// "Complete this meal" results — the remaining menu dishes that would round
/// out the current selection. Tap one to add it to the selection.
class _CompleteMealSheet extends StatelessWidget {
  final List<MealCompletionSuggestion> suggestions;
  final ValueChanged<MenuItem> onPick;

  const _CompleteMealSheet({required this.suggestions, required this.onPick});
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.restaurant_menu_rounded, size: 20, color: accent),
                const SizedBox(width: 8),
                Expanded(
                  child: Text(
                    'Complete this meal',
            ...
            Text(
              'From this menu, what would round out your plate',
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        const SizedBox(height: 2),
                                        Text(
                                          '${s.item.calories.round()} cal · '
                                          '${s.item.proteinG.toStringAsFixed(0)}g P · ${s.reason}',
                                          style: TextStyle(
                                            fontSize: 11.5,
                                            color: colors.textSecondary,
                                          ),
