# Trademark Takedown Response — Ready-to-Send Drafts

Last updated: 2026-04-25.
Owner: ChetanG.
Use these if a "FITWIZ" / Lin Jiangjuan IP complaint arrives via Apple's
App Store IP form or Play Console's Removal Request form.

> **Window:** Apple gives ~5 business days; Play gives ~7 days. Submit
> within 48 hours regardless. Apps stay listed during the dispute on
> first-pass; takedown only happens if the complainant escalates.

---

## TL;DR facts you'll cite (memorize these)

| Fact | Value |
|---|---|
| Our app store name | `FitWiz: Workout & Meal Coach` |
| Our iOS bundle ID | `com.aifitnesscoach.aiFitnessCoach` |
| Our Android applicationId | `com.aifitnesscoach.app` |
| Our registered domain | `fitwiz.us` (since 2024 — verify exact reg date in your registrar) |
| Our category | Health & Fitness |
| Our product | AI-built workout + nutrition planner with set-by-set adaptation |
| Complainant's mark | FITWIZ, USPTO Reg #7111694, Serial 97441093, IC 009 |
| Complainant's first use | 2022-05-10 (vs. ours, fill in your first-use date) |
| Complainant's owner | LIN, JIANGJUAN (Ningbo, China; organized Canada) |
| Complainant's domain | `fitwiz.app` |
| Complainant's app | iOS only, last updated Oct 2025, App Store listing under "FitWiz" |
| Goods description critical phrase | "Downloadable computer application software for tablets, computers, **vehicle intelligent terminals**, smartwatches and mobile phones, namely…" — verify the *namely* clause from TSDR before sending. If it does NOT name fitness/exercise/health, lead with that. |

---

## Apple App Store IP complaint response (Form: https://www.apple.com/legal/internet-services/itunes/appstorenotices/)

```
Subject: Response to IP complaint — FitWiz: Workout & Meal Coach
       (Apple ID [your iOS App Store app ID])

Dear App Store Legal,

We received notice of an intellectual property complaint regarding our
app FitWiz: Workout & Meal Coach. We respectfully dispute the claim of
likelihood of confusion, and provide the following facts for your
review.

1. Different developer, different applicationId, different domain
   - Our developer account: [your registered Apple Developer name]
   - Our iOS bundle ID: com.aifitnesscoach.aiFitnessCoach
   - Our domain: fitwiz.us (registered [your registrar date])
   - Complainant's iOS bundle ID: [their bundle ID, look up on App Store]
   - Complainant's domain: fitwiz.app

2. Different product positioning and goods/services description
   The complainant's USPTO Registration #7111694 (Serial 97441093) lists
   IC 009 goods including "vehicle intelligent terminals, smartwatches
   and mobile phones, namely [insert the namely clause from TSDR]."
   Our app is a consumer-facing AI workout and meal-plan coach
   targeting strength training, hypertrophy, and nutrition logging.
   These are distinct goods and channels of trade.

3. We have added a clear product descriptor to our App Store name
   ("Workout & Meal Coach") to eliminate any consumer-confusion ambiguity.
   Our app icon, color palette, and visual identity are visually
   distinct from complainant's app.

4. We do not use, reference, or link to fitwiz.app anywhere in the
   product, the App Store listing, or marketing materials.

We are open to good-faith discussion with the complainant if they
believe further differentiation is appropriate. Please advise if any
additional information would help your review.

Sincerely,
ChetanG
[your legal entity, e.g. FitWiz, Inc. (Delaware)]
[your address]
[your contact email]
```

---

## Google Play removal-dispute response (Form: Play Console → Policy → Action against your app → Submit appeal)

```
Subject: Appeal — IP complaint on FitWiz: Workout & Meal Coach
       (com.aifitnesscoach.app)

We respectfully appeal the takedown notice and submit the following:

1. Distinct package and developer identity
   - Our applicationId: com.aifitnesscoach.app (verifiably distinct
     from any package owned by the complainant).
   - Our developer account: [your Google Play developer name]
   - Our domain: fitwiz.us (separate from complainant's fitwiz.app)

2. Distinct product positioning
   The Play Store name "FitWiz: Workout & Meal Coach" includes a
   descriptive disambiguator. Our app is a consumer fitness planner
   focused on workout programming and nutrition coaching. The
   complainant's USPTO Reg #7111694 IC 009 description includes
   "vehicle intelligent terminals" and other non-fitness goods, with
   a "namely" clause that [insert TSDR language].

3. No use of complainant's mark or assets
   We do not reproduce the complainant's logo, copy, screenshots, or
   any identifying material. We do not link to fitwiz.app. We do not
   reference the complainant's app in marketing.

4. Visual differentiation
   Our app icon, color system, and screenshots are independently
   designed and do not resemble the complainant's listing.

We respectfully request restoration of the listing. We are willing to
engage with the complainant in good faith and to make further
clarifying changes (e.g. additional descriptor language) if doing so
would resolve the concern.

Sincerely,
ChetanG
[legal entity, address, contact]
```

---

## What to gather BEFORE you need to send (do today)

- [ ] Screenshot of complainant's App Store listing — capture title,
      icon, screenshots, developer name, "More from this developer".
      Save with date stamp.
- [ ] Save the full TSDR record for Reg #7111694 as a PDF — go to
      https://tsdr.uspto.gov/#caseNumber=97441093 → Documents → Save.
      The "namely" clause matters; the registration certificate +
      specimen-of-use show what they actually sell.
- [ ] Pull your fitwiz.us domain registration date from your registrar
      (Namecheap / Squarespace / etc) — establishes your good-faith
      use timeline.
- [ ] Save your first-use-anywhere date for the FitWiz brand — your
      first commit referencing "FitWiz", earliest TestFlight build,
      first marketing post, etc. This becomes your priority claim if
      you ever counter-file (which we are not recommending today).
- [ ] Confirm legal entity: if you don't have an LLC yet, form one
      this week. Personal liability shield matters disproportionately
      with a known IP overlap.

---

## What NOT to do

- Don't argue trademark validity. Their registration is final on the
  Principal register; arguing validity requires a TTAB cancellation
  proceeding which costs $50K+ and 18+ months. Argue *non-confusion*
  on facts.
- Don't claim "FITWIZ" is generic or descriptive. It's neither. The
  Principal register issuance settled that.
- Don't admit knowledge of their mark before a specific date. Stick to
  factual differentiation.
- Don't engage publicly (Twitter, Reddit, etc) about the dispute —
  every public statement becomes evidence.
- Don't use "Coach" in copy that resembles complainant's tagline.
- Don't add additional FITWIZ assets to the codebase, marketing, or
  swag while the dispute is open.
