...
class TestGeminiService:
    ...
    @pytest.mark.asyncio
    async def test_extract_intent_add_exercise(self, mock_gemini_service):
        """Test intent extraction for adding exercise."""
        result = await mock_gemini_service.extract_intent("add push-ups to my workout")

        assert result.intent == CoachIntent.ADD_EXERCISE
        assert "push-ups" in result.exercises

    @pytest.mark.asyncio
    async def test_extract_intent_remove_exercise(self, mock_gemini_service):
        """Test intent extraction for removing exercise."""
        result = await mock_gemini_service.extract_intent("remove squats please")

        assert result.intent == CoachIntent.REMOVE_EXERCISE

    @pytest.mark.asyncio
    async def test_extract_intent_swap_workout(self, mock_gemini_service):
        """Test intent extraction for swapping workout."""
        result = await mock_gemini_service.extract_intent("I want a different workout today")

        assert result.intent == CoachIntent.SWAP_WORKOUT

    @pytest.mark.asyncio
    async def test_extract_intent_modify_intensity(self, mock_gemini_service):
        """Test intent extraction for modifying intensity."""
        result = await mock_gemini_service.extract_intent("make it easier please")

        assert result.intent == CoachIntent.MODIFY_INTENSITY
        assert result.modification == "easier"

    @pytest.mark.asyncio
    async def test_extract_intent_report_injury(self, mock_gemini_service):
        """Test intent extraction for reporting injury."""
        result = await mock_gemini_service.extract_intent("my shoulder hurts")

        assert result.intent == CoachIntent.REPORT_INJURY
        assert result.body_part == "shoulder"

    @pytest.mark.asyncio
    async def test_extract_intent_question(self, mock_gemini_service):
        """Test intent extraction for general question."""
        result = await mock_gemini_service.extract_intent("what's the best way to warm up?")

        assert result.intent == CoachIntent.QUESTION

    @pytest.mark.asyncio
    async def test_chat_returns_response(self, mock_gemini_service):
        """Test that chat returns a response."""
        response = await mock_gemini_service.chat("Hello, coach!")
    ...
    @pytest.mark.asyncio
    async def test_chat_with_system_prompt(self, mock_gemini_service):
        """Test chat with system prompt."""
        response = await mock_gemini_service.chat(
    ...
    @pytest.mark.asyncio
    async def test_chat_with_conversation_history(self, mock_gemini_service):
    ...
    def test_get_embedding(self, mock_gemini_service):
        """Test embedding generation."""
        embedding = mock_gemini_service.get_embedding("test text")

        assert embedding is not None
        assert isinstance(embedding, list)
        assert len(embedding) == 768  # Gemini text-embedding-004 dimension

    def test_get_coach_system_prompt(self, mock_gemini_service):
        """Test system prompt generation."""
        prompt = mock_gemini_service.get_coach_system_prompt("User context here")
