...
{(f'ACTION REQUIRED: This is a restaurant menu (or food delivery app menu). Call analyze_multi_food_images with s3_keys and mime_types from media_refs and analysis_mode="menu". The user typed: "{state.get("user_message","")}". After the tool returns the OCR-ed dishes, your final user-visible reply MUST directly address that question by referencing 1-3 specific items by name from the OCR (with calories and a one-sentence reason tied to the user macros and goals). Never default to a generic meal suggestion that ignores the menu.') if state.get('media_content_type') == 'food_menu' and state.get('media_refs') else ''}
{(f'ACTION REQUIRED: This is a restaurant/delivery menu image (food delivery apps like DoorDash, Uber Eats count too). Call analyze_food_image with the image_base64 and user_message="OCR every visible dish/item on this menu: extract the name, price if shown, and estimated calories+protein+carbs+fat per typical serving. Return JSON." After the tool returns, your final user-visible reply MUST directly address the user actual question ("{state.get("user_message","")}") by referencing 1-3 specific items by name from that OCR (with calories and a one-sentence reason). Never produce a generic meal suggestion that ignores the menu.') if state.get('media_content_type') == 'food_menu' and not state.get('media_refs') and state.get('image_base64') else ''}
{(f'ACTION REQUIRED: This is a buffet spread. Call analyze_multi_food_images with s3_keys and mime_types from media_refs and analysis_mode="buffet". The user typed: "{state.get("user_message","")}". After the tool returns, address that question directly using the identified dishes.') if state.get('media_content_type') == 'food_buffet' and state.get('media_refs') else ''}
{(f'ACTION REQUIRED: This is a buffet spread image. Call analyze_food_image with the image_base64 and user_message="Identify every visible dish on this buffet with estimated calories+protein+carbs+fat per typical serving. Return JSON." Then address the user actual question ("{state.get("user_message","")}") using those dishes.') if state.get('media_content_type') == 'food_buffet' and not state.get('media_refs') and state.get('image_base64') else ''}

{(f'FALLBACK MENU RECOMMENDATION: An image is attached, media_content_type is missing/unknown/food_plate, AND the user is asking for a recommendation/decision (e.g. "what should I eat", "which is best", "recommend", "pick", "suggest", "order"). Treat the image as a menu/options list: call analyze_food_image with image_base64 and user_message="OCR every dish/item visible in this image with name, price if shown, and estimated calories+protein+carbs+fat per serving. Return JSON list." Then, in your final reply, recommend 1-3 specific items by name from that OCR with calories and a one-sentence reason tied to the user macros, directly answering the user question ("{state.get("user_message","")}"). Never answer with a generic meal suggestion that ignores the image.') if state.get('image_base64') and (not state.get('media_content_type') or state.get('media_content_type') in ('unknown', 'food_plate')) and any(k in state.get('user_message', '').lower() for k in ('what should i eat', 'what to eat', 'which should i', 'recommend', 'pick', 'suggest', 'best option', 'which is', 'order')) else ''}

CRITICAL CO-REASONING RULE: Always read the user typed message ("{state.get('user_message', '')}") together with the attached media. The text is the question; the image is the evidence. If the user asked a recommendation/decision question and you can see distinct food items in the image, you MUST OCR the items and pick from them by name in your reply. Never produce a generic meal answer (e.g. "grilled chicken and quinoa") that ignores the attached image content. If you genuinely cannot identify what the image shows, say so explicitly and ask one clarifying question rather than fabricating a recommendation.
