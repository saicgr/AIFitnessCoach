...
from .rag_service_helpers import (  # noqa: F401
    NutritionRAGService,
)
        ...
        try:
            ...
            self.changes_collection.add(
                ...
                metadatas=[{
                    ...
                    "goals": ",".join(str(g) for g in goals) if goals else "",
                    "motivations": ",".join(str(m) for m in motivations) if motivations else "",
                    ...
                    "workout_days": ",".join(str(d) for d in workout_days) if workout_days else "",
                    "equipment": ",".join(str(e) for e in equipment) if equipment else "",
                    "dumbbell_count": dumbbell_count or 2,
                    "kettlebell_count": kettlebell_count or 1,
                    "focus_areas": ",".join(str(f) for f in focus_areas) if focus_areas else "",
                    "injuries": ",".join(str(i) for i in injuries) if injuries else "",
