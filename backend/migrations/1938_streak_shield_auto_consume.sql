-- ============================================================================
-- Migration 1938: Streak Shield auto-consume on missed day
-- ============================================================================
-- BUG FIX: Inventory screen copy says "Used automatically when you miss a day"
-- but no backend code actually consumed shields on streak breaks. Users saw
-- shields in inventory expecting auto-save → streak still broke → trust break.
--
-- This migration patches `process_daily_login` so that when a user's streak
-- would break (ELSE branch), we check for streak_shield in user_consumables:
--   - If count >= 1 → consume 1 shield, PRESERVE current_streak, flag save.
--   - Else → existing break behavior.
--
-- Adds `streak_saved_by_shield: boolean` to the JSONB response so the
-- frontend can fire the "We saved your streak" celebration dialog.
-- ============================================================================
...
DECLARE
  ...
  v_streak_saved_by_shield BOOLEAN := false;
  v_shields_remaining INT := 0;
  v_new_streak INT;
...
BEGIN
  ...
  ELSIF v_streak_record.last_login_date = v_today THEN
    -- Already logged in today — idempotent
  ...
  ELSE
    -- Streak would break. Check shields BEFORE breaking.
    -- NEW (migration 1938): auto-consume one streak_shield if available.
    IF use_consumable(p_user_id, 'streak_shield') THEN
      -- Shield saved the streak: increment normally as if it had been
      -- a consecutive day.
      v_streak_saved_by_shield := true;
      v_saved_streak_count := v_streak_record.current_streak;
      v_new_streak := v_streak_record.current_streak + 1;
      ...
      RETURNING * INTO v_streak_record;
    ELSE
      -- No shield available — original break behavior
  ...
  -- Remaining shield count (for UI display)
  SELECT COALESCE(quantity, 0) INTO v_shields_remaining
  FROM user_consumables
  WHERE user_id = p_user_id AND item_type = 'streak_shield';
  ...
  RETURN jsonb_build_object(
    ...
    'streak_saved_by_shield', v_streak_saved_by_shield,
    'shields_remaining', v_shields_remaining,
    'saved_streak_count', v_saved_streak_count,  -- pre-save streak length, for celebration copy
    ...
    'message',
      CASE
        ...
        WHEN v_streak_saved_by_shield THEN
          'Streak Shield used! Your ' || v_saved_streak_count || '-day streak is safe.'
...
'Processes daily login. Migration 1938: auto-consumes a streak_shield on missed days if available, preserving the streak. Returns streak_saved_by_shield flag + shields_remaining for frontend celebration + UI display.';
