-- ============================================================================
-- Migration 1934: Beef up L3/L7/L10 reward bundles
-- ============================================================================
-- L3 (first "retention check" — day 1-2 for committed users):
--     old: 1x Fitness Crate
--     new: 2x Fitness Crate + 1x Streak Shield + 1x 2x XP Token
--
-- L7 (mid-week 1 — biggest churn cliff):
--     old: 1x Fitness Crate + 1x Streak Shield
--     new: 2x Fitness Crate + 2x Streak Shield + 1x 2x XP Token + 1x Premium Crate
--
-- L10 ("Iron Will" first major — bump across the board):
--     old: 3x 2x XP Token + 2x Fitness Crate + 1x Premium Crate
--     new: 5x 2x XP Token + 3x Fitness Crate + 2x Premium Crate + 2x Streak Shield
-- ============================================================================
  ...
  FOR v_level IN (p_old_level + 1)..p_new_level LOOP
    ...
    ELSE
      CASE (v_level % 10)
        ...
        WHEN 3 THEN
          -- BEEFED UP: first multi-reward per-level
          PERFORM add_consumable(p_user_id, 'fitness_crate', 2);
          PERFORM add_consumable(p_user_id, 'streak_shield', 1);
          PERFORM add_consumable(p_user_id, 'xp_token_2x', 1);
          v_items := jsonb_build_array(
