-- Migration 1948: broaden the leaderboard pool from "active THIS WEEK" to
-- "active in last 30 days". Users who haven't worked out this week but did
-- recently still appear in rankings (with weekly metric 0), so the board
-- feels populated at small scale. Dormant accounts (30+ days inactive)
-- stay out — we don't inflate percentiles against ghosts.
-- Applied to prod Supabase 2026-04-18 via MCP.
--
-- Weekly metric math (XP / volume / streak) is unchanged. Only the set of
-- ranked users changes: EXISTS filter on workout_logs was
-- `DATE_TRUNC('week', completed_at) = p_week_start`, now is
-- `completed_at > NOW() - INTERVAL '30 days'`.
--
-- Functions patched:
--   * compute_user_percentile
--   * get_near_you_leaderboard
--   * snapshot_weekly_leaderboard (cron archive)
