-- Migration 1887: Fix daily login response field names AND column references
-- Migration 1884 regressed fixes from migration 229/232:
--   1. Response field names: daily_bonus->daily_xp, streak_bonus->streak_milestone_xp, +multiplier
--   2. Column references: max_streak->longest_streak, today_claimed_at->last_daily_bonus_claimed
--   3. xp_events columns: start_time/end_time->start_at/end_at
--
-- Verified actual column names via information_schema:
--   user_login_streaks: longest_streak (NOT max_streak), last_daily_bonus_claimed (NOT today_claimed_at)
--   xp_events: start_at/end_at (NOT start_time/end_time)

-- Drop existing function first (return type change from JSON to JSONB requires this)
...
Migration 1887: Fixed response field names (daily_xp, streak_milestone_xp, longest_streak, multiplier)
AND column references (longest_streak not max_streak, last_daily_bonus_claimed not today_claimed_at,
start_at/end_at not start_time/end_time). Verified against actual DB schema.';
