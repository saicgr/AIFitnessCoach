-- Zealova rebrand — replace "Welcome to FitWiz!" strings in process_daily_login.
--
-- Migrations 210, 229, 1901 each defined process_daily_login with hardcoded
-- "Welcome to FitWiz!" copy. The migration files were rewritten to "Zealova"
-- but existing databases still hold the old string in pg_proc.
-- This migration is idempotent: it re-CREATE OR REPLACEs the function body
-- copied verbatim from migration 1901 (the latest definition) with Zealova
-- substituted. Pure brand-string change, zero logic change.
--
-- Safe to re-run.
...
Migration 2034: Replaced "Welcome to FitWiz!" with "Welcome to Zealova!" for rebrand.
No logic change from migration 1901.';
