-- Migration: 134_sync_onboarding_to_preferences.sql
-- Description: Fix calculate_nutrition_metrics to also save nutrition_goals,
-- and create function to sync fasting preferences from onboarding
-- Created: 2026-01-05

-- ============================================================================
-- PART 1: UPDATE calculate_nutrition_metrics TO SAVE nutrition_goals
-- ============================================================================
...
BEGIN
    ...
    -- ==========================================
    -- Update nutrition_preferences (NOW WITH nutrition_goals!)
    -- ==========================================
    ...
    VALUES (
        p_user_id,
        COALESCE(p_nutrition_goals, ARRAY['maintain']),
        v_primary_goal,
        v_bmr,
        v_tdee,
        v_target_calories,
    ...
    ON CONFLICT (user_id) DO UPDATE SET
        nutrition_goals = COALESCE(p_nutrition_goals, ARRAY['maintain']),
        nutrition_goal = v_primary_goal,
    ...
    v_result := jsonb_build_object(
        ...
        'bmr', v_bmr,
        'tdee', v_tdee,
        'nutrition_goals', to_jsonb(COALESCE(p_nutrition_goals, ARRAY['maintain']))
    );
...
-- ============================================================================
-- PART 2: CREATE FUNCTION TO SYNC FASTING PREFERENCES FROM ONBOARDING
-- ============================================================================

CREATE OR REPLACE FUNCTION sync_fasting_from_onboarding(
    p_user_id UUID,
    p_interested_in_fasting BOOLEAN,
    p_fasting_protocol VARCHAR DEFAULT NULL
)
RETURNS JSONB
SECURITY DEFINER
SET search_path = public
...
BEGIN
    ...
    IF p_interested_in_fasting IS NOT TRUE THEN
    ...
    -- Normalize protocol format (convert "16:8" style to standard format)
    v_protocol := COALESCE(p_fasting_protocol, '16:8');

    -- Handle custom protocol format from onboarding (e.g., "custom:16:8")
    IF v_protocol LIKE 'custom:%' THEN
        -- Extract the hours from custom:XX:YY format
        -- For now, just use the default protocol if it's custom
        v_protocol := '16:8';
    END IF;

    -- Insert or update fasting_preferences
    INSERT INTO fasting_preferences (
        user_id,
        default_protocol,
        fasting_onboarding_completed,
    ...
    ON CONFLICT (user_id) DO UPDATE SET
        default_protocol = v_protocol,
        fasting_onboarding_completed = TRUE,
        onboarding_completed_at = COALESCE(fasting_preferences.onboarding_completed_at, NOW()),
        updated_at = NOW();
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION sync_fasting_from_onboarding TO authenticated;
GRANT EXECUTE ON FUNCTION sync_fasting_from_onboarding TO service_role;
