-- 2422_daily_login_bypass_trust_halving.sql
-- Finding #357 (Every XP goal pays a different amount than it advertises --
-- and the same goal pays differently on different days).
--
-- Root cause (the daily_login / streak_milestone slice of it): process_daily_login
-- (see migration 2400) calls award_xp() for 'daily_login' and 'streak_milestone'
-- WITHOUT p_bypass_trust=true. award_xp (161_xp_system.sql / 231_level_up_rewards.sql
-- / 1901_fix_xp_levels.sql) applies a 0.5x anti-fraud multiplier to every
-- award for trust_level=1 accounts unless bypassed -- so the SAME formula
-- (xp_bonus_templates.daily_login.base_xp=25 * LEAST(streak, 7)) pays a
-- different real amount depending on the account's trust_level at the moment
-- of the call: 25 XP one day, 12 XP (FLOOR(25*0.5)) four days later for the
-- same streak day. Evidence: xp_transactions read (14 rows summing correctly
-- to user_xp.total_xp -- the ledger math is honest, only the PER-ROW amount
-- disagrees with the base formula).
--
-- This is the exact bug class already fixed for workout_complete XP
-- (crud_completion.py -- "100 advertised = 100 paid" via bypass_trust) applied
-- to the other two award_xp call sites inside process_daily_login. Both
-- daily_login and streak_milestone are already non-farmable (gated by the
-- once-per-day user_login_streaks row + the streak-day check itself), so
-- the anti-fraud halving buys no real fraud protection here -- it only makes
-- the advertised number a lie for any account that has not yet reached
-- trust_level 2.
--
-- Body is byte-for-byte migration 2400's process_daily_login, with
-- p_bypass_trust=true added to the daily_login and streak_milestone award_xp calls
-- only (first_login already passes v_is_early_adopter as its bypass flag --
-- unchanged). No other line changed.
...
BEGIN
  ...
  IF v_daily_bonus > 0 THEN
    BEGIN
      PERFORM award_xp(p_user_id, v_daily_bonus, 'daily_login', NULL, 'Daily check-in bonus', false, true);
    EXCEPTION WHEN OTHERS THEN
      INSERT INTO xp_transactions (user_id, xp_amount, source, description, created_at)
...
Migration 2422 (E2E #357): daily_login and streak_milestone award_xp calls now
pass p_bypass_trust=true so the advertised streak-scaled amount always equals
the paid amount, regardless of the account''s trust_level. Migration 2400
(E2E #66) behavior (daily_login suppressed on the first-ever call) is
unchanged.';

-- VERIFY: select xp_amount, source from xp_transactions where user_id = '<test-user-uuid>' and source in ('daily_login','streak_milestone') order by created_at desc limit 5; -- amounts should equal xp_bonus_templates.base_xp * LEAST(streak, max_streak_multiplier) with no 0.5x reduction for trust_level=1 accounts
