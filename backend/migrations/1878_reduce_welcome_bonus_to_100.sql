-- Migration 1878: Reduce Welcome Bonus to 100 XP
-- Previous: 250 XP (Migration 228) → landed new users at Level 6
-- New: 100 XP → lands new users at Level 3 (95 cumulative for L3, 145 for L4)
-- This creates better early progression pacing
--
-- RUN THIS IN SUPABASE SQL EDITOR (anon key can't update xp_bonus_templates due to RLS)

-- 1. Update the bonus template
UPDATE xp_bonus_templates
SET base_xp = 100,
    description = 'Welcome bonus for new users (525 XP for first 100 early adopters)'
WHERE bonus_type = 'first_login';

-- 2. Update the process_daily_login function with new default
...
DECLARE
  ...
  v_normal_bonus INT := 100;          -- Standard welcome bonus (reduced from 250)
BEGIN
  ...
  IF v_streak_record.user_id IS NULL THEN
    ...
    -- Award first login bonus (525 XP for early adopters, 100 XP for others)
...
Migration 1878: Welcome bonus reduced to 100 XP (525 XP for first 100 early adopters)';
