-- Migration: Fix exercise_library_cleaned data quality
-- Created: 2026-03-08
-- Purpose: Filter out exercises missing critical metadata (body_part, target_muscle)
--          and known junk entries from the cleaned view. This ensures both app queries
--          AND ChromaDB indexing only receive well-formed exercise data.
...
COMMENT ON VIEW exercise_library_cleaned IS 'Deduplicated exercise library - prioritizes entries with video+image, filters out entries missing body_part/target_muscle';
