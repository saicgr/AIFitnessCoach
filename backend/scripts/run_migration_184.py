#!/usr/bin/env python3
"""
Run migration 184: Add estimated_duration_minutes column to workouts table

This migration:
1. Adds estimated_duration_minutes column to workouts table
2. Adds check constraint for valid duration range (1-480 minutes)
3. Adds column documentation comment
"""
...
def run_migration(client):
    ...
    migration_path = os.path.join(
        ...
        "migrations",
        "184_add_estimated_duration_minutes.sql"
    )

    print(f"\n📄 Reading migration 184: add_estimated_duration_minutes")
    ...
    try:
        ...
        print(f"✅ Migration 184 completed!")
        return True
...
def main():
    ...
    if run_migration(client):
        print("\n✅ Migration 184 completed successfully!")
        print("   Added estimated_duration_minutes column to workouts table")
    else:
        print("\n⚠️  Migration needs manual execution in Supabase SQL Editor.")
        print("   Copy and paste the SQL from:")
        print("   - backend/migrations/184_add_estimated_duration_minutes.sql")
