#!/usr/bin/env python3
"""Run migration 1884: Fix new user level (start at Level 1, not 7)."""
import os, sys
...
def run_migration():
    ...
    file_path = migrations_dir / "1884_fix_new_user_level.sql"
    ...
    print(f"\n{'='*60}")
    print(f"Running: 1884_fix_new_user_level.sql")
    print(f"  Sets first_login bonus to 0 XP so new users start at Level 1")
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(sql)
        conn.commit()
        print(f"  SUCCESS - New user level fix applied")
