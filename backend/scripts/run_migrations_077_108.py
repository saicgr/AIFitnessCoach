#!/usr/bin/env python3
"""
Migration Runner Script for migrations 077-108.
...
def get_migration_files():
    """Get all migration files from 077 to 108 in numerical order."""
    ...
    # Files 077-099
    for num in range(77, 100):
        pattern = migrations_dir / f"0{num}_*.sql"
        files = glob.glob(str(pattern))
        all_files.extend(files)

    # Files 100-108
    for num in range(100, 109):
        pattern = migrations_dir / f"{num}_*.sql"
        files = glob.glob(str(pattern))
        all_files.extend(files)

    # Sort by filename to handle multiple files with same number prefix
    all_files.sort()
