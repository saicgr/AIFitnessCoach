...
async def auto_generate_workout(user_id: str, target_date: date, gym_profile_id: Optional[str] = None, selected_days: Optional[List[int]] = None, adjacent_day_exercises: Optional[List[str]] = None, batch_offset: int = 0, _retry_count: int = 0, user_tz: str = "UTC"):
    ...
    try:
        ...
        # Per-day overrides from the user's weekly plan
        # (users.preferences.workout_day_overrides). ALWAYS consulted now
        # (no env gate — only activates for days the user explicitly set),
        # fail-open. Focus also flows via generate_workout's prompt block, but
        # here we thread DURATION + INTENSITY into the request so they truly
        # drive the workout (not just an advisory prompt), align focus, and
        ...
        try:
            ...
            if _day_ov:
                if _day_ov.get("gym_profile_id"):
                    gym_profile_id = _day_ov["gym_profile_id"]
                _f = _day_ov.get("focus")
                if _f:
                    focus_for_day = _f
                    workout_type = infer_workout_type_from_focus(_f)
                if _day_ov.get("duration_min"):
                    _per_day_duration = _day_ov["duration_min"]
                _intensity = _day_ov.get("intensity")
                if _intensity:
                    # The picker uses "moderate"; generation expects "medium".
                    _per_day_intensity = (
                ...
                logger.info(
                    ...
                    f"intensity={_per_day_intensity} gym={gym_profile_id}"
                )
