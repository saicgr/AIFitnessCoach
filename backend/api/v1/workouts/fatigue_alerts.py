...
async def check_fatigue(request: FatigueCheckRequest) -> FatigueCheckResponse:
    ...
    try:
        ...
        logger.error(f"[Fatigue Check] Error: {e}", exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def get_next_set_preview(
    ...
    try:
        ...
        logger.error(f"[Next Set Preview] Error: {e}", exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def check_fatigue_with_preview(
    ...
    try:
        ...
        logger.error(f"[Combined Check] Error: {e}", exc_info=True)
        raise HTTPException(
            status_code=500,
