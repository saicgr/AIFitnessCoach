...
// Keep src/lib/toolStats.ts in sync with the registry so every "N free
// tools" mention in marketing copy (hero, nav, footer, FAQ, marquee) uses
// one accurate number that updates automatically when tools are added.
async function writeToolStats(tools) {
  const out = `// AUTO-GENERATED by scripts/generate-seo.mjs on every build — do not edit
...
export const FREE_TOOL_COUNT = ${tools.length};
`;
  await fs.writeFile(path.join(ROOT, 'src/lib/toolStats.ts'), out);
}
...
async function main() {
  ...
  await writeToolStats(tools);
  console.log(
    `[seo] sitemap.xml: ${sitemapCount} URLs · llms.txt: ${llmsCount} tools listed · toolStats: ${tools.length}`,
  );
}
