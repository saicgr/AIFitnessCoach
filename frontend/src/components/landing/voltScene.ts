// Vanilla three.js hero backdrop. A single dim ember flow-field — the
// hero is a product stage and the DOM phone is the star. The big 3D moment
// lives in explodedScene.ts (the scroll-driven device teardown section).
...
void main() {
  ...
  vec3 bg = vec3(0.031, 0.024, 0.020);
  vec3 moss = vec3(0.20, 0.09, 0.02);
  vec3 volt = vec3(1.0, 0.478, 0.0);

  vec3 col = bg;
  col = mix(col, moss, haze);
  col += volt * filament * 0.16;
...
export function createVoltScene(canvas: HTMLCanvasElement): VoltSceneHandle | null {
  ...
  const bgMaterial = new RawShaderMaterial({
  ...
  bgScene.add(new Mesh(bgGeometry, bgMaterial));

  const mouseTarget = new Vector2(0, 0);
  const mouseLerped = new Vector2(0, 0);
  ...
  const resize = () => {
    ...
    bgUniforms.uResolution.value.set(
  ...
  const loop = (now: number) => {
    ...
    mouseLerped.lerp(mouseTarget, 0.05);
